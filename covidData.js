...
function callback(error, response, html) {
  if (!error) {
    ...
    let todayVaccine = $(numberVaccine[2])
      .text()
      .replace("(", "")
      .replace(")", "");
    let tillTodayVaccine = $(numberVaccine[1])
      .text()
      .replace("(", "")
      .replace(")", "");
    ...
    console.log(
      "COVID-19 VACCINATION INDIA STATS updated by " + date + " " + time
    );
    console.log();
    ...
    console.log(
      "Active Cases             : " +
        indian_System_separator(activeCase.substring(0, 7))
    );
    console.log(
      "New Cases                : " +
        indian_System_separator(activeCase.substring(13, 17))
    );
    console.log();
    ...
    console.log(
      "Total Discharged         : " +
        indian_System_separator(dischargeCase.substring(0, 8))
    );
    console.log(
      "Discharged Today         : " +
        indian_System_separator(dischargeCase.substring(14, 20))
    );
    console.log();
    ...
    console.log(
      "Total Death              : " +
        indian_System_separator(deathCase.substring(0, 6))
    );
    console.log(
      "Death Today              : " +
        indian_System_separator(deathCase.substring(12, 16))
    );
    console.log(
      "-----------------------------------------------------------------------------"
    );
...
async function main() {
  ...
  let age = await tab.$$(".form-check.nomargright label", {
  ...
  if (
    (await tab.$(
      ".main-slider-wrap.col.col-lg-3.col-md-3.col-sm-3.col-xs-12"
    )) !== null
  ) {
    let slotS = await tab.$$(".vaccine-box.vaccine-box1.vaccine-padding", {
      waitUntil: "networkidle2",
    });
    ...
    console.log(
      "       DETAILS ABOUT SLOT AVAILABILITY AND VACCINE CENTRES IN YOUR AREA "
    );
    ...
    if (count == 0) {
      console.log(
        "---->>> SORRY, NO SLOT AVAILABLE FOR BOOKING IN YOUR AREA! TRY AGAIN AFTER A MOMENT <<<----"
      );
    } else {
      console.log(
        "---->>>  " +
          count +
          "  SLOTS ARE AVAILABLE FOR BOOKING IN YOUR AREA , HURRY! BOOK YOUR SLOT AND GET A JAB <<----"
      );
    }
    ...
    let centreName = await tab.$$(
      ".main-slider-wrap.col.col-lg-3.col-md-3.col-sm-3.col-xs-12 .center-name-title",
      { waitUntil: "networkidle2" }
    );
    let centreAdd = await tab.$$(
      ".main-slider-wrap.col.col-lg-3.col-md-3.col-sm-3.col-xs-12 .center-name-text",
      { waitUntil: "networkidle2" }
    );
    console.log(
      "      ---->> " +
        centreName.length +
        " VACCINATION CENTERS IS AVAILABLE NEAR YOU <<----"
    );
    console.log();
